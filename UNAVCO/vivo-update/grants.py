    ...
    if bindings:
        for record in bindings:
            ...
            if "id" in record:
                ...
                if award_id[:4] in ('EAR-','AGS-','DUE-','GEO-','IIA-','PLR-'):
                    award_id = award_id[4:]
    ...
    if grants:
        ...
        for grant in grants:
            ...
            if nsf_id not in q_info:
                ...
                g.add((D[award_uri], VIVO.sponsorAwardId, Literal(grant['id'])))
                g.add((D[award_uri], VIVO.totalAwardAmount, Literal("${:,.2f}"
                        .format(float(grant['fundsObligatedAmt'])))))
                if 'agency' in grant:
                    if grant['agency'] == 'NSF':
                        g.add((D[award_uri], VIVO.assignedBy, D[NAT_ID]))
                ...
                grant['startDate'] = datetime.strptime(
                                                grant['startDate'], '%m/%d/%Y')
                grant['startDate'] = grant['startDate'].strftime(
                                                            '%Y-%m-%dT%H:%M:%S')     
                ...
                if 'expDate' in grant:
                    ...
                    g.add((D[end_uri], VIVO.dateTimePrecision, 
                            VIVO.yearMonthDayPrecision))
                    grant['expDate'] = datetime.strptime(
                                            grant['expDate'], '%m/%d/%Y')
                    grant['expDate'] = grant['expDate'].strftime(
                                            '%Y-%m-%dT%H:%M:%S')     
                ...
                if not admin_uri:
                    log.info('Awardee '+admin_org+' not found in Connect UNAVCO'
                            ' database. Adding triples.')
                    admin_uri = uri_gen('org')
                    g.add((D[admin_uri], RDF.type, FOAF.Organization))
                    g.add((D[admin_uri], RDFS.label, 
                            Literal(admin_org)))
                ...
                if 'coPDPI' in grant:
                    for person in grant['coPDPI']:
                        ...
                        g.add((D[co_pi_role_uri], VIVO.relatedBy, D[award_uri]))
...
if len(g)>0:
    try:                    
        with open("rdf/grant-update-"+timestamp+"-in.ttl", "w") as f:    
            ...
            log.info('Wrote RDF to rdf/grant-update-'+timestamp+'-in.ttl in '+
                        args.format+' format.')
    except IOError:
        # Handle the error.
...
if len(gout)>0:
    try:
        with open("rdf/grant-update-"+timestamp+"-out.ttl", "w") as fout:
            ...
            log.info('Wrote RDF to rdf/grant-update-'+timestamp+'-out.ttl in '+
                        args.format+' format.')            
    except IOError:
        # Handle the error.
        log.exception("Failed to write RDF file. "
