...
//        if ((!this.supportEdit) && (this.editMode == 'edit' || this.editMode == 'repair')) {
    ...
    initAutocomplete: function(selectedObj) {
        ...
        $(selectedObj).autocomplete({
            ...
            source: function(request, response) {
                ...
                $.ajax({
                    url: customForm.acUrl,//this should be the external autocomplete
                    dataType: 'json',
                    data: {
                    	serviceURI:customForm.serviceURI,//this should be the service uri passed in through the form
                        term: request.term,
                    ...
                    complete: function(xhr, status) {
                        ...
                        //Certain lookup services may need to manipulate the label
                        customForm.updateLabelsForDisplay(filteredResults);
    ...
    showAutocompleteSelection:function(item, selectedObj) {
    	customForm.showAutocompleteSelection_base(item, selectedObj);
    },
    //the mechanism we are using isn't strictly inheritance
    //it is more copying additional methods to the BASE
    //which means that we can't call the 'super' method since it will just overwrite it
    showAutocompleteSelection_base: function(item, selectedObj) {
...
//This is so we have a consistent method across all the types of objects
var getExternalLookup_base= {
		getLookup:function() {
			return customForm;
		}
	}